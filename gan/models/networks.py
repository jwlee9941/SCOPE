...
import numpy as np
from .convnext import ConvNeXtGenerator
from .stylegan_networks import StyleGAN2Discriminator, StyleGAN2Generator, TileStyleGAN2Discriminator
from .pixelinstancenorm import PixelInstanceNorm2d
###############################################################################
# Helper Functions
###############################################################################
...
def get_norm_layer(norm_type='instance'):
    ...
    elif norm_type == 'pixel-instance':
        norm_layer = functools.partial(PixelInstanceNorm2d, affine=False)
    elif norm_type == 'none':
        def norm_layer(x):
            return Identity()
    else:
...
def define_G(input_nc, output_nc, ngf, netG, norm='batch', use_dropout=False, init_type='normal',
    ...
    if netG == 'resnet_9blocks':
        ...
        net = ResnetGenerator(input_nc, output_nc, ngf, tanh=False, norm_layer=norm_layer, use_dropout=use_dropout, no_antialias=no_antialias, no_antialias_up=no_antialias_up, n_blocks=9, opt=opt)
        ...
        net = ConvNeXtGenerator(input_nc, output_nc)    
    else:
...
class PatchSampleF(nn.Module):
    ...
    def forward(self, feats, num_patches=64, patch_ids=None):
        #print("input for netF: ", type(feats), len(feats))
        return_ids = []
        return_feats = []
...
class ResnetGenerator(nn.Module):
    ...
    def __init__(self, input_nc, output_nc, ngf=64, tanh=True, norm_layer=nn.BatchNorm2d, use_dropout=False, n_blocks=6, padding_type='reflect', no_antialias=False, no_antialias_up=False, opt=None):
...
class NLayerDiscriminator(nn.Module):
    ...
    def __init__(self, input_nc, ndf=64, n_layers=3, norm_layer=nn.BatchNorm2d, no_antialias=False):
        ...
        # x_for_plot = torch.zeros(1,3,256,256)
        # make_dot(self.model(x_for_plot), params=dict(list(self.model.named_parameters())), show_saved=True ).render("netF_architecture",format="png")

    def forward(self, input):
        """Standard forward."""
        #print("input shape of netD:", input.size())
        return self.model(input)
...
class PatchDiscriminator(NLayerDiscriminator):
    ...
    def forward(self, input):
        print("input of patchgan discriminator: ", input.size())
...
if __name__ == '__main__':
    net = ResnetGenerator(3, 3, 64)
    for i, x in enumerate(net.model):
        print(i, x)