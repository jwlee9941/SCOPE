...
class DCLModel(BaseModel):
    ...
    def modify_commandline_options(parser, is_train=True):
        ...
        parser.add_argument('--decay_D_A', type=float, default=1.0, help='decay weight for GAN loss：GAN(G(X))')
        parser.add_argument('--lambda_NCE', type=float, default=2, help='weight for NCE loss: NCE(G(X), X)')
        parser.add_argument('--lambda_IDT', type=float, default=1.0, help='weight for l1 identical loss: (G(X),X)')

        #add lambda_recon
        parser.add_argument('--lambda_RECON', type=float, default=0.0, help='weight for reconstruction loss')
        ...
        #parser.add_argument('--flip_equivariance', action='store_true',
        #            help="Enforce flip-equivariance as additional regularization.")
    ...
    def __init__(self, opt):
        ...
        self.netG_A = networks.define_G(opt.input_nc, opt.output_nc, opt.ngf, opt.netG, opt.normG,
                                        ...
                                        opt.no_antialias_up, self.gpu_ids, opt) ## G1: A->B
        self.netG_B = networks.define_G(opt.input_nc, opt.output_nc, opt.ngf, opt.netG, opt.normG,
                                        not opt.no_dropout, opt.init_type, opt.init_gain, opt.no_antialias,
                                        opt.no_antialias_up, self.gpu_ids, opt) ## G2: B->A
        self.netF1 = networks.define_F(opt.input_nc, opt.netF, opt.normG,
                                       ...
                                       opt) ## MLP Layer for patch NCE Loss
        ...
        if self.isTrain:
            ...
            self.netD_B = networks.define_D(opt.output_nc, opt.ndf, opt.netD,
                                            opt.n_layers_D, opt.normD, opt.init_type, opt.init_gain, opt.no_antialias,
                                            self.gpu_ids, opt)
            self.fake_A_pool = ImagePool(opt.pool_size)  # create image buffer to  previously generated images
            ...
            self.criterionRecon = torch.nn.L1Loss('sum').to(self.device)
            self.optimizer_G = torch.optim.Adam(itertools.chain(self.netG_A.parameters(), self.netG_B.parameters()),
                                                lr=opt.lr, betas=(opt.beta1, opt.beta2))
            self.optimizer_D = torch.optim.Adam(itertools.chain(self.netD_A.parameters(), self.netD_B.parameters()),
                                                lr=opt.lr * self.opt.decay_D_A, betas=(opt.beta1, opt.beta2))
    ...
    def compute_G_loss(self):
        ...
        if self.opt.lambda_RECON > 0.0:
            self.recon_loss_G_A = self.criterionRecon(self.real_A, self.fake_B) * self.opt.lambda_RECON
            self.recon_loss_G_B = self.criterionRecon(self.real_B, self.fake_A) * self.opt.lambda_RECON
        else:
            self.recon_loss_G_A = 0.0
            self.recon_loss_G_B = 0.0

        self.loss_G = (self.loss_G_A + self.loss_G_B) * 0.5 + loss_NCE_both + (self.recon_loss_G_B + self.recon_loss_G_A) * 0.5
        return self.loss_G
